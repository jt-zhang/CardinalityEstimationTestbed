...
df_forest = pd.read_csv("../train-test-data/forest_&_power-data-sql/forest.csv", sep=',', escapechar='\\', encoding='utf-8', low_memory=False, quotechar='"')
...
f2 = open("../train-test-data/forest_&_power-data-sql/forest.sql",'w')
...
df_power = pd.read_csv("../train-test-data/forest_&_power-data-sql/power.csv", sep=',', escapechar='\\', encoding='utf-8', low_memory=False, quotechar='"')
...
for key, value in dictpower.items():
    for i in range(1, len(value)):
        locals()[value[0] + '_' + value[i]] = list(locals()['df_'+key][value[i]].unique())
f2 = open("../train-test-data/forest_&_power-data-sql/power.sql",'w')
...
f = open('../train-test-data/forest_&_power-data-sql/' + 'forest' + 'test.sql')
queries = f.readlines() 
f2 = open('../train-test-data/forest_&_power-data-sql/' + 'forest' + 'test2.sql', 'w')
...
f = open('../train-test-data/forest_&_power-data-sql/' + 'power' + 'test.sql')
queries = f.readlines() 
f2 = open('../train-test-data/forest_&_power-data-sql/' + 'power' + 'test2.sql', 'w')
for query in queries:
