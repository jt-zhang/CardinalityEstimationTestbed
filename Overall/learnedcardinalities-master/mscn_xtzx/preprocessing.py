...
def get_col_statistics(cols, data_dir):
    alias2table = {'t1':'auth_user', 't2':'student_courseenrollment', 't3':'organization_account_userorgprofile', 't4':'auth_userprofile'}
    names = []
    cards = []
    distinct_nums = []
    mins = []
    ...
    for col in cols:
        names.append(col)
        print (col)
        col_materialize = pd.read_csv(data_dir+'/'+alias2table[col.split('.')[0]]+'.csv',quotechar='"',escapechar='\\',error_bad_lines=False,low_memory=False)[col.split('.')[1]]
    ...
    statistics = pd.DataFrame(data = {'name': names, 'min': mins, 'max': maxs, 'cardinality': cards, 'num_unique_values': distinct_nums})
    statistics.to_csv(min_max_file, index=False)

def select_samples(data_dir):
    table2alias = {'auth_user': 't1', 'student_courseenrollment': 't2', 'organization_account_userorgprofile': 't3', 'auth_userprofile': 't4'}
    samples = {}
    for table, alias in table2alias.items():
        samples[alias] = pd.read_csv('{}/{}'.format(data_dir, table+'.csv'),quotechar='"',escapechar='\\',error_bad_lines=False,low_memory=False).sample(n=1000)
    return samples
