        ...
        for line in lines:
            ...
            if key not in pattern2truecard:
                ...
                for t in full_tables:
                    print (t)
                    if t != 'title':
                        datas.append(pd.read_csv('{}/{}.csv'.format(datasets_dir, t),quotechar='"',escapechar='\\',error_bad_lines=False,low_memory=False).groupby(['movie_id']).size().reset_index(name='counts').set_index('movie_id'))
                total = 0
