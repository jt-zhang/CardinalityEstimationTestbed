        ...
        for line in lines:
            ...
            if key not in pattern2truecard:
                ...
                for t in full_tables:
                    print (t)
                    if t != 'auth_user':
                        datas.append(pd.read_csv('{}/{}.csv'.format(datasets_dir, t),quotechar='"',escapechar='\\',error_bad_lines=False,low_memory=False).groupby(['user_id']).size().reset_index(name='counts').set_index('user_id'))
                total = 0
...
if __name__ == '__main__':
    ...
    for k, v in pattern2training.items():
        ...
        np.savetxt('/home/jintao/CardinalityEstimationBenchmark/quicksel/test/java/edu/illinois/quicksel/resources/{}/{}.assertion'.format(args.output_dir, k), data, delimiter=",")
        vecs = []
        for _ in range(int(len(v[0]) / 2)):
            vecs.append(0.0)
            vecs.append(1.0)
        vecs.append(1.0)
        np.savetxt('/home/jintao/CardinalityEstimationBenchmark/quicksel/test/java/edu/illinois/quicksel/resources/{}/{}.permanent'.format(args.output_dir, k), np.array([vecs]), delimiter=",")
